...
// import { getMembers, updateMemberRole, removeMember } from "../../services/playlistShare.service";

export default function ManageMembersModal({ isOpen, onClose, playlistId }) {
    const [members, setMembers] = useState([]);
    const [isLoading, setIsLoading] = useState(true);
    const [actionLoading, setActionLoading] = useState(null); // tracks which user is being updated
    ...
    useEffect(() => {
        ...
        const fetchMembers = async () => {
            setIsLoading(true);
            try {
                // const data = await getMembers(playlistId);
                // setMembers(data);
            } catch (err) {
        ...
        <Modal isOpen={isOpen} onClose={onClose} title="Manage Collaborators" maxWidth="max-w-lg">
            <div className="flex flex-col gap-4">
                            ...
                            {members.map((member) => (
                                <div key={member.user._id} className="flex items-center justify-between rounded-lg border border-gray-100 p-3 bg-gray-50">
                                    <div className="flex items-center gap-3">
                                        <img 
                                            src={member.user.profilePic?.url || "https://via.placeholder.com/40"} 
                ...
                <div className="mt-4 flex justify-end border-t border-gray-100 pt-4">
                    <Button onClick={onClose} bgColor="bg-gray-100" textColor="text-gray-700">
                        Done
                    </Button>
                </div>
            </div>
