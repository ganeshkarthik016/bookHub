...
export default function EditProfileModal({ currentUser }) {
    ...
    const handleInfoSubmit = async (e) => {
        e.preventDefault();
        setError(""); setSuccess(""); setIsUpdatingInfo(true);
        try {
    ...
    const handlePicSubmit = async (e) => {
        ...
        setError(""); setSuccess(""); setIsUpdatingPic(true);
        try {
    ...
    return (
        <Modal isOpen={isOpen} onClose={() => { setError(""); setSuccess(""); dispatch(closeModal()); }} title="Edit Profile" maxWidth="max-w-lg">
                ...
                <form onSubmit={handlePicSubmit} className="flex flex-col items-center gap-4 rounded-xl border border-gray-200 bg-gray-50 p-6">
                    ...
                    {profilePic && (
                        <Button type="submit" disabled={isUpdatingPic} className="flex items-center gap-2 text-sm">
                            {isUpdatingPic ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Save Picture
                        </Button>
                    )}
                </form>
                <form onSubmit={handleInfoSubmit} className="flex flex-col gap-4">
                    <Input label="Full Name" placeholder="e.g., John Doe" value={userFullName} onChange={(e) => setUserFullName(e.target.value)} />
                    <div className="flex flex-col gap-1">
                        <label className="text-sm font-medium text-gray-700">Bio</label>
                        <textarea value={bio} onChange={(e) => setBio(e.target.value)} placeholder="Tell us about yourself..." rows="3" className="w-full resize-none rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
                    </div>
                    <div className="mt-2 flex justify-end gap-3 border-t border-gray-100 pt-4">
                        <Button type="button" bgColor="bg-gray-100" textColor="text-gray-700" onClick={() => dispatch(closeModal())}>Cancel</Button>
                        <Button type="submit" disabled={isUpdatingInfo || (!userFullName.trim() && !bio.trim())}>{isUpdatingInfo ? "Saving..." : "Save Details"}</Button>
                    </div>
                </form>
            </div>
        </Modal>
